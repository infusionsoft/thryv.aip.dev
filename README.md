...
```bash
docker run -d --name aip-site-dev aip-site-dev
```

This will start the container in detached mode. You can verify it's running with:

```bash
docker ps
```

#### 4. Execute the Serve Script

Once the container is up and running, execute the `serve.sh` script:
...
```bash
docker stop aip-site-dev
```

To remove the container:

```bash
docker rm aip-site-dev
```
